...
export function ExpenseCategoryManagerDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Organize your expenses into categories.</DialogDescription>
          </DialogHeader>
                  ...
                  {editingId === c.id ? (
                    ...
                    <>
                      <span className="text-sm">
                        {c.name}{' '}
                        <span className="text-xs text-muted-foreground">({c._count?.expenses ?? 0})</span>
                      </span>
      ...
      <ConfirmDialog
        ...
        description={`Expenses in "${deleteTarget?.name}" will become uncategorized, not deleted.`}
        confirmLabel="Delete"
